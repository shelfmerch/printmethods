...
export default function Stores() {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <aside className="hidden lg:block w-64 border-r bg-muted/10 p-6 space-y-8 sticky top-0 h-screen overflow-y-auto">
        <Link to="/" className="flex items-center space-x-2">
          <img src={logo} alt="logo" className="w-40 rounded-3xl shadow-2xl" />
        </Link>
        ...
        <div className="pt-4 border-t">
          <p className="text-sm text-muted-foreground mb-2 px-2">Signed in as</p>
          <p className="text-sm font-medium px-2 truncate mb-4">
            {user?.email}
          </p>
          <Button
            variant="ghost"
              ...
              {stores.length === 0 ? (
                ...
                <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                  {stores.map((store) => (
                    <Card
                      key={store.id}
                      className="p-6 flex flex-col justify-between gap-4 border-l-4 border-l-primary"
                    >
                      ...
                      {store.builderLastPublishedAt && (
                        <p className="text-xs text-muted-foreground">
                          Last published:{" "}
                          {new Date(store.builderLastPublishedAt).toLocaleDateString()}
                        </p>
                      )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              <div onClick={() => handleConnectChannel("Etsy")}>
                <ChannelButton
                  name="Etsy"
                  icon={
                    <span className="text-[#F1641E] font-serif font-bold text-xl">
                      Etsy
                    </span>
                  }
                />
              ...
              <div onClick={() => handleConnectChannel("TikTok")}>
                <ChannelButton
                  name="TikTok"
                  icon={
                    <span className="font-bold text-xl bg-clip-text text-transparent bg-gradient-to-r from-[#25F4EE] to-[#FE2C55]">
                      TikTok
                    </span>
                  }
                />
              ...
              <div onClick={() => handleConnectChannel("Squarespace")}>
                <ChannelButton
                  name="Squarespace"
                  icon={<span className="font-bold text-lg">Squarespace</span>}
                />
              </div>
              <div onClick={() => handleConnectChannel("Wix")}>
                <ChannelButton name="Wix" icon={<span className="font-bold text-xl">Wix</span>} />
            ...
            <div className="grid gap-6">
              <Card className="p-8 bg-muted/30 border-none shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                ...
                <Button
                  ...
                  onClick={handleLaunchPopupStore}
