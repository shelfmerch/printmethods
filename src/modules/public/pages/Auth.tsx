...
// import { getSafeRedirect } from '@/shared/utils/authUtils';
...
const Auth = () => {
  ...
  useEffect(() => {
    ...
    // Shopify embedded auth entry: build returnTo explicitly from shop + host.
    // This prevents accidental relative redirects like "shopify/app" which would become "/auth/shopify/app".
    if (shopParam && hostParam) {
      const clean = `/shopify/app?shop=${encodeURIComponent(shopParam)}&host=${encodeURIComponent(hostParam)}`;
      resolvedReturnTo = clean;
      sessionStorage.setItem('returnTo', clean);
      console.log('[Auth] returnTo saved from shop+host params:', clean);
    } else if (returnToParam) {
      // Non-Shopify entry: accept only absolute in-app paths (or absolute URLs that can be converted safely)
      let clean = returnToParam;
      if (clean.startsWith('http://') || clean.startsWith('https://')) {
        try { clean = new URL(clean).pathname + new URL(clean).search + new URL(clean).hash; } catch { }
      }
      if (clean.startsWith('/')) {
        resolvedReturnTo = clean;
        sessionStorage.setItem('returnTo', clean);
      ...
      if (fullPath) {
        ...
        // location.state.from is generated by React Router; keep it absolute-only
